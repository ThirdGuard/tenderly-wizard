...
const scope_curve_1 = require("./scope-curve");
// npx hardhat run src/whitelist/execute-curve-whitelist.ts --network localhost
const run = async () => {
    console.log(`Start whitelisting CURVE on ${hardhat_1.network.name}...`);
    ...
    // Curve whitelist class will gather all the necessary scoping and crafting of the multisend transaction
    const curveWhitelistSecurity = new scope_curve_1.CurveWhitelist(process.env.INVESTMENT_ROLES_ADDRESS, ledgerSigner);
    // build() will construct the unsigned transaction
    const whitelistPopTx = await curveWhitelistSecurity.build(process.env.ACCESS_CONTROL_ROLES_ADDRESS, process.env.INVESTMENT_SAFE_ADDRESS);
