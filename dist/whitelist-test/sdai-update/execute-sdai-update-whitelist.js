...
const scope_sdai_update_1 = require("./scope-sdai-update");
// npx hardhat run src/whitelist/execute-sdai-whitelist.ts --network localhost
const run = async () => {
    console.log(`Start whitelisting sDAI Update (Lite PSM USDC A) on ${hardhat_1.network.name}...`);
    ...
    // usdc sdai whitelist class will gather all the necessary scoping and crafting of the multisend transaction
    const sDaiUpdateWhitelistSecurity = new scope_sdai_update_1.SdaiUpdateWhitelist(process.env.INVESTMENT_ROLES_ADDRESS, ledgerSigner);
    // build() will construct the unsigned transaction
    const whitelistPopTx = await sDaiUpdateWhitelistSecurity.build(process.env.ACCESS_CONTROL_ROLES_ADDRESS, process.env.INVESTMENT_SAFE_ADDRESS);
