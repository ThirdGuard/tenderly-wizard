...
import { LedgerSignerV6 } from "./ledger-adapter";
...
import { getChainConfig } from "./roles-chain-config";
...
// Make LedgerSigner compatible with ContractRunner by using our adapter
type CompatibleSigner = SignerWithAddress | LedgerSignerV6;
 ...
 * @param rolesVersion Version of the roles contract being used
...
export async function executeWhitelistV2(
    ...
    const calls = await applyTargets(
        asHexString(MANAGER_ROLE_ID_V2),
        targets,
        {
            chainId,
