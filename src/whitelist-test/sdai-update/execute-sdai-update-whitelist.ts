...
import { SdaiUpdateWhitelist } from './scope-sdai-update';

// npx hardhat run src/whitelist/execute-sdai-whitelist.ts --network localhost
const run = async () => {
    console.log(`Start whitelisting sDAI Update (Lite PSM USDC A) on ${hre.network.name}...`)
    ...
    const sDaiUpdateWhitelistSecurity = new SdaiUpdateWhitelist(process.env.INVESTMENT_ROLES_ADDRESS, ledgerSigner);
