...
import { CurveWhitelist } from "./scope-curve";

// npx hardhat run src/whitelist/execute-curve-whitelist.ts --network localhost
const run = async () => {
    console.log(`Start whitelisting CURVE on ${hre.network.name}...`)
    ...
    const curveWhitelistSecurity = new CurveWhitelist(process.env.INVESTMENT_ROLES_ADDRESS, ledgerSigner);
